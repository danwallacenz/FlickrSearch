...
struct OrderedDictionary<KeyType:Hashable, ValueType> {
    ...
    //  WARNING:    For type inference to work, the compiler requires that the type of an expression be unambiguous. When multiple methods exist with the same argument types but different return types, the caller needs to be specific. Adding a method in Swift can be a build-breaking change, so be aware!
